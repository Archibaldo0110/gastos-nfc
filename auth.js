...
const CATEGORIAS = ["Comida", "Golosinas", "Transporte", "Ocio", "Compras", "Ropa", "Higiene", "Hogar", "Útiles", "Servicios"];
...
function signOut() {
  ...
  localStorage.removeItem('gastosnfc_resumen_ready_v2');
  accessToken = null;
  tokenExpiry = 0;
}
...
async function getOrCreateSheet() {
  ...
  localStorage.setItem('gastosnfc_sheet_id', sheetId);
  localStorage.setItem('gastosnfc_resumen_ready_v2', '1');
  return sheetId;
}
...
async function asegurarResumen() {
  if (localStorage.getItem('gastosnfc_resumen_ready_v2')) return;
