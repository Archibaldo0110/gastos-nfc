...
const CATEGORIAS = ["Comida", "Transporte", "Ocio", "Compras", "Útiles", "Servicios"];
...
function signOut() {
  ...
  localStorage.removeItem('gastosnfc_resumen_ready');
  accessToken = null;
  tokenExpiry = 0;
}
...
async function getOrCreateSheet() {
  ...
  localStorage.setItem('gastosnfc_sheet_id', sheetId);
  localStorage.setItem('gastosnfc_resumen_ready', '1');
  return sheetId;
}
...
async function asegurarResumen() {
  if (localStorage.getItem('gastosnfc_resumen_ready')) return;
